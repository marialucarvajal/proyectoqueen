public class Tablero {
    ...
    public boolean moverPiezaDoble(String origen, String destino, String colorJugador) {
        ...
        int dx = Math.abs(d[0] - o[0]);
        int dy = Math.abs(d[1] - o[1]);

        if (dx > 2 || dy > 2) {
            System.out.println("Movimiento inválido: solo hasta 2 espacios.");
            return false;
        }
        ...
        if (origenC.getPieza() == null || !origenC.getPieza().getColor().equals(colorJugador)) return false;
        if (destinoC.getPieza() != null) return false;
