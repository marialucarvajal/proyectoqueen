public class Tablero {
    ...
    public boolean esMovimientoValido(int[] origen, int[] destino, boolean esDoble) {
        int difFilas = Math.abs(destino[0] - origen[0]);
        int difColumnas = Math.abs(destino[1] - origen[1]);

        if (!esDoble && difFilas <= 1 && difColumnas <= 1) return true;

        return esDoble && ((difFilas == 2 && difColumnas == 0) || (difFilas == 0 && difColumnas == 2));
    }
